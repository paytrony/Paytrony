...
-- Multi-mint quantity support for package purchases.

ALTER TABLE public.payment_intents
  ADD COLUMN IF NOT EXISTS quantity int NOT NULL DEFAULT 1
    CHECK (quantity BETWEEN 1 AND 20);

-- New purchase_package variant accepting a quantity.
...
BEGIN
  ...
  -- Idempotent replay: same base key + qty → returns the prior mint set.
...
-- Existing 3-arg variant now delegates to the new 4-arg with quantity=1.
CREATE OR REPLACE FUNCTION public.purchase_package(
  _user_id uuid,
  _amount numeric,
...
DECLARE
  res jsonb;
BEGIN
  res := public.purchase_package(_user_id, _amount, _idempotency_key, 1);
  RETURN res;
END;
$$;

REVOKE ALL ON FUNCTION public.purchase_package(uuid, numeric, text, int) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.purchase_package(uuid, numeric, text, int) TO authenticated, service_role;
